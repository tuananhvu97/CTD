...
void compileVarDecls(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      objCount++;
      objList = (Object** )realloc(objList, objCount*sizeof(Object));
      objList[objCount-1] = varObj;
      ...
      for(i=0; i<objCount; i++){
	  	objList[i]->varAttrs->type = varType;
	  	declareObject(objList[i]);
	  }      
      eat(SB_SEMICOLON);
    } while (lookAhead->tokenType == TK_IDENT);
  } 
...
Type* compileLValue(void) {
  ...
  case OBJ_PARAMETER:
	...
	else if(var->paramAttrs->kind == PARAM_VALUE){
...
void compileCallSt(void) {
  ...
  if (isPredefinedProcedure(proc)) {
    ...
    genPredefinedProcedureCall(proc);
  	...
  	genINT(4);
    compileArguments(proc->procAttrs->paramList);
    genDCT(proc->procAttrs->paramCount+4);
...
void compileForSt(void) {
  ...
  if(lookAhead->tokenType == SB_LPAR){
  	...
  	eat(SB_SEMICOLON);
  	fjInstruction = genFJ(DC_VALUE);
  	jumpInstruction = genJ(DC_VALUE);
  	
  	back = getCurrentCodeAddress();
  	varType = compileLValue();
  	eat(SB_EQ);
  	type = compileExpression();
  	checkTypeEquality(varType, type);
  	...
  	updateJ(jumpInstruction, getCurrentCodeAddress());
  	eat(SB_RPAR);
  	compileStatement();
  	genJ(back);
  	updateFJ(fjInstruction, getCurrentCodeAddress());
