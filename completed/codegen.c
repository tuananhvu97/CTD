...
void genProcedureCall(Object* proc) {
  // TODO
  int p = computeNestedLevel(proc->procAttrs->scope->outer);
  int q = proc->procAttrs->codeAddress;
  genCALL(p, q);
}
...
void genFunctionCall(Object* func) {
  // TODO
  int p = computeNestedLevel(func->funcAttrs->scope->outer);
  int q = func->funcAttrs->codeAddress;
  genCALL(p, q);
}
